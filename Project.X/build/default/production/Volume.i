...
void update_vol_motor()
{
    ...
    while (volume != curPotmeterVol) {
        ...
        if (volume > curPotmeterVol) {
            PORTCbits.RC0 = 1;
            PORTCbits.RC1 = 0;
        } else if (volume < curPotmeterVol) {
            PORTCbits.RC0 = 0;
            PORTCbits.RC1 = 1;
        }
    }
...
int is_deadzone(int currentStep, int adcResult)
{
    int turnPoint = step_size * currentStep;

    int deadzoneLow = turnPoint - ((1023) * 0.01);
    int deadzoneHigh = turnPoint + ((1023) * 0.01);

    return adcResult > deadzoneLow && adcResult < deadzoneHigh;
}
...
int get_volume(unsigned int ad_result)
{
    if (ad_result == 0 || ad_result < step_size) {
        ...
        unsigned int currentStep = ad_result / step_size;
